...
public class PerspectiveModelData {
    public static final Codec<PerspectiveModelFile> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    ...
                    Codec.unboundedMap(ModelTransformationMode.CODEC, Identifier.CODEC).fieldOf("perspectives").forGetter(unbakedPerspectiveModel -> unbakedPerspectiveModel.perspectiveModels)
            ).apply(instance, PerspectiveModelFile::new)
    );
    private final ModelKey fallback;
    private final Map<ModelTransformationMode, ModelKey> perspectiveModels;

    public PerspectiveModelData(ModelKey fallback, Map<ModelTransformationMode, ModelKey> perspectiveModels) {
    ...
    public record PerspectiveModelFile(Identifier fallback, Map<ModelTransformationMode, Identifier> perspectiveModels) {}
    ...
    public static class Builder {
        ...
        Builder(ModelKey fallback) {
            this.fallback = fallback;
            this.perspectiveModels = new EnumMap<>(ModelTransformationMode.class);
        }

        public Builder withModel(ModelTransformationMode transformationMode, ModelKey identifier) {
